...
func _ready() -> void:
	...
	line_edit_session.text_changed.connect(on_session_text)
	line_edit_username.text_changed.connect(on_username_text)
...
func on_session_text(new_text: String):
	if new_text != '':
		button_join_tube.disabled = false
	
func on_username_text(new_text: String):
